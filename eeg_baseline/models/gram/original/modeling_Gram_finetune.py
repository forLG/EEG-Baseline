# --------------------------------------------------------
# This file defines the model for the fine-tuning stage.
# It adapts the pre-trained Gram model (encoder) for downstream classification tasks.
#
# Key differences from the pre-training model:
# 1. The MAE decoder is completely removed as it's not needed for classification.
# 2. A new linear classification head (`cls_head`) is added on top of the encoder.
# 3. The forward pass processes the entire input sequence without any masking.
# 4. The model outputs classification logits derived from the [CLS] token.
# 5. It includes logic to load weights from a pre-trained encoder checkpoint,
#    initializing the encoder while leaving the new classification head random.
# --------------------------------------------------------
# References:
...
if __name__=='__main__':
    # Example: How to load the pretrained model
