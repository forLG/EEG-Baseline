...
class TUSZLoader(torch.utils.data.Dataset):
    def __init__(self, root, files, sampling_rate=200):
        self.root = root
        self.files = files
        self.default_rate = 200
...
def prepare_HMC_dataset(root):
    ...
    # prepare training and test data loader
    train_dataset = HMCLoader(os.path.join(root, "train"), train_files)
    test_dataset = HMCLoader(os.path.join(root, "test"), test_files)
    val_dataset = HMCLoader(os.path.join(root, "val"), val_files)
    print(len(train_files), len(val_files), len(test_files))
    return train_dataset, test_dataset, val_dataset
